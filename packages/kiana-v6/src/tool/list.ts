...
export const listTool = defineTool("list", {
  ...
  async execute(params, ctx) {
    const searchPath = path.resolve(ctx.workingDirectory, params.path || ".")
    ...
    const files: string[] = []
    for await (const file of listFiles({ cwd: searchPath, glob: ignoreGlobs })) {
      files.push(file)
    ...
    return {
      title: path.relative(ctx.workingDirectory, searchPath) || ".",
      metadata: {
        count: files.length,
